...
enable_startup_banner() {
    local rc S="# >>> shell-wizard dashboard >>>" E="# <<< shell-wizard dashboard <<<"
    echo -e "\n${CYAN}--> Enabling system dashboard on terminal startup...${NC}"
    if ! command -v fastfetch &>/dev/null && ! command -v neofetch &>/dev/null; then
        echo -e "${YELLOW}[!] Neither fastfetch nor neofetch is installed.${NC}"
        read -p "Install it now? [y/N]: " ANS
        if [[ "$ANS" =~ ^[Yy]$ ]]; then
            install_cli_tools
        fi
        if ! command -v fastfetch &>/dev/null && ! command -v neofetch &>/dev/null; then
            echo -e "${RED}[!] Still not installed, so the banner was not enabled.${NC}"
            return 1
        fi
    fi
    for rc in "${HOME}/.zshrc" "${HOME}/.bashrc"; do
        ...
        if grep -qF "$S" "$rc"; then
            echo -e "  ${YELLOW}[i] Already enabled in:${NC} ~/${rc##*/}"
            continue
        fi
        printf '\n%s\n[[ $- == *i* ]] && { if command -v fastfetch &>/dev/null; then fastfetch; elif command -v neofetch &>/dev/null; then neofetch; fi; }\n%s\n' "$S" "$E" >> "$rc"
        if grep -qF "$S" "$rc"; then
            echo -e "  ${GREEN}[✔] Startup banner enabled in:${NC} ~/${rc##*/}"
        else
            echo -e "  ${RED}[!] Failed to write to ~/${rc##*/}${NC}"
        fi
    done
    echo -e "${CYAN}[💡] Open a new terminal tab to see it.${NC}"
}

disable_startup_banner() {
    local rc S="# >>> shell-wizard dashboard >>>" E="# <<< shell-wizard dashboard <<<"
    echo -e "\n${CYAN}--> Disabling system dashboard on terminal startup...${NC}"
    for rc in "${HOME}/.zshrc" "${HOME}/.bashrc"; do
        [[ -f "$rc" ]] || continue
        sed -i "/${S}/,/${E}/d" "$rc"
        sed -i -e '/^# Launch dashboard on terminal startup$/,+1d' "$rc"
        echo -e "  ${GREEN}[✔] Startup banner removed from:${NC} ~/${rc##*/}"
    done
}
...
manage_cli_tools() {
    while true; do
        ...
        echo -e "  ${GREEN}[5]${NC} Disable System Dashboard on Startup"
        echo -e "  ${GREEN}[0]${NC} Back to Main Menu"
        echo -e "\n===================================================================="
        read -p "Select choice [0-5]: " C_CHOICE
