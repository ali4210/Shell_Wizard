...
apply_starship_preset() {
    local PRESET_NAME="$1" NAME tmp err
    mkdir -p "$STARSHIP_CONFIG_DIR"

    if ! command -v starship &>/dev/null; then
        echo -e "${RED}[!] Starship is not installed. Run option 1 first.${NC}"
        return 1
    fi

    case $PRESET_NAME in
        "nerd-font")   NAME="nerd-font-symbols" ;;
        "gruvbox")     NAME="gruvbox-rainbow" ;;
        "tokyo-night") NAME="tokyo-night" ;;
        "bracketed")   NAME="bracketed-segments" ;;
        "plain")       NAME="plain-text-symbols" ;;
        *) echo -e "${RED}[!] Unknown preset '${PRESET_NAME}'.${NC}"; return 1 ;;
    esac

    echo -e "${CYAN}--> Applying Starship preset '${PRESET_NAME}'...${NC}"
    tmp="$(mktemp)"
    if err=$(starship preset "$NAME" -o "$tmp" --force 2>&1) && [[ -s "$tmp" ]]; then
        [[ -f "$STARSHIP_FILE" ]] && cp "$STARSHIP_FILE" "${STARSHIP_FILE}.bak"
        cp "$tmp" "$STARSHIP_FILE"
        rm -f "$tmp"
        echo -e "${GREEN}[✔] Preset '${PRESET_NAME}' applied to ~/.config/starship.toml!${NC}"
        echo -e "${CYAN}[💡] It loads on your next prompt. If not, run ${BOLD}exec zsh${NC}${CYAN}.${NC}"
    else
        rm -f "$tmp"
        echo -e "${RED}[!] Could not apply '${PRESET_NAME}'. Your old config was kept.${NC}"
